class MutSR_SRHUDKillingFloor extends HUD_StoryMode;
...
var MutSR_ClientPerkRepLink ClientRep;
...
function DrawPlayerInfo(Canvas C, Pawn P, float ScreenLocX, float ScreenLocY)
{
	...
	Class'MutSR_SRScoreBoard'.Static.TextSizeCountry(C,P.PlayerReplicationInfo,XL,YL);
	Class'MutSR_SRScoreBoard'.Static.DrawCountryName(C,P.PlayerReplicationInfo,ScreenLocX-(XL * 0.5),ScreenLocY-(YL * 0.75));

	OffsetX = (36.f * VeterancyMatScaleFactor * 0.6) - (HealthIconSize + 2.0);

	if ( Class<MutSR_SRVeterancyTypes>(KFPlayerReplicationInfo(P.PlayerReplicationInfo).ClientVeteranSkill)!=none &&
		 KFPlayerReplicationInfo(P.PlayerReplicationInfo).ClientVeteranSkill.default.OnHUDIcon!=none )
	{
		TempLevel = Class<MutSR_SRVeterancyTypes>(KFPlayerReplicationInfo(P.PlayerReplicationInfo).ClientVeteranSkill).Static.PreDrawPerk(C,
...
simulated function DrawHudPassA (Canvas C)
{
	...
	if( KFPlayerReplicationInfo(PawnOwnerPRI)!=None )
		SV = Class<MutSR_SRVeterancyTypes>(KFPlayerReplicationInfo(PawnOwnerPRI).ClientVeteranSkill);
	...
	if ( SV!=None )
	{
		...
		if( ClientRep!=None && (TempLevel+1)<ClientRep.MaximumLevel )
		{
			...
			Class'MutSR_SRScoreBoard'.Static.DrawProgressBar(C,TempX,TempY-TempSize*0.12f,TempSize*2.f,TempSize*0.1f,VisualProgressBar);
		}
...
simulated final function DrawPortraitX( Canvas C )
{
	...
	if ( PortraitPRI != None )
	{
		...
		Class'MutSR_SRScoreBoard'.Static.TextSizeCountry(C,PortraitPRI,XL,YL);
		if ( XL > PortraitWidth )
		{
		...
		Class'MutSR_SRScoreBoard'.Static.DrawCountryName(C,PortraitPRI,C.ClipY / 256 - PortraitWidth * PortraitX + 0.5 * (PortraitWidth - XL), 0.5 * (C.ClipY + PortraitHeight) + 0.06 * PortraitHeight,Abbrev);
	}
