class MutSR_SRPerkSelectList extends KFPerkSelectList;
...
function DrawPerk(Canvas Canvas, int CurIndex, float X, float Y, float Width, float Height, bool bSelected, bool bPending)
{
	...
	ST = Class'MutSR_ClientPerkRepLink'.Static.FindStats(Canvas.Viewport.Actor);
	if( ST==None )
		return;
...
{
	Return MenuOwner.ActualHeight()/8/*ItemCount*/-0.5;
} */
