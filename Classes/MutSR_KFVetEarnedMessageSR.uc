class MutSR_KFVetEarnedMessageSR extends CriticalEventPlus
	abstract;
    ...
    )
{
	...
	if( Class<MutSR_SRVeterancyTypes>(OptionalObject)!=None )
	{
		A = Canvas.DrawColor.A;
		Class<MutSR_SRVeterancyTypes>(OptionalObject).Static.PreDrawPerk(Canvas,Switch,M1,M2);
    ...
    )
{
	...
	if( MutSR_SRHUDKillingFloor(P.myHUD)!=None && KFPlayerReplicationInfo(P.PlayerReplicationInfo)!=None
	 && OptionalObject!=None && KFPlayerReplicationInfo(P.PlayerReplicationInfo).ClientVeteranSkill==OptionalObject )
	{
		...
		MutSR_SRHUDKillingFloor(P.myHUD).LevelProgressBar = 1.f;
		MutSR_SRHUDKillingFloor(P.myHUD).NextLevelTimer = P.Level.TimeSeconds+1.f;
	}
