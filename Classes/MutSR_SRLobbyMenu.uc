...
//
//-----------------------------------------------------------
class MutSR_SRLobbyMenu extends LobbyMenu;
...
function AddPlayer( KFPlayerReplicationInfo PRI, int Index, Canvas C )
{
	...
	if ( PRI.ClientVeteranSkill != none )
	{
		...
		if( Class<MutSR_SRVeterancyTypes>(PRI.ClientVeteranSkill)!=None )
		{
			Class<MutSR_SRVeterancyTypes>(PRI.ClientVeteranSkill).Static.PreDrawPerk(C,PRI.ClientVeteranSkillLevel,PlayerBoxes[Index].PlayerPerk.Image,M);
...
function DrawPerk(Canvas Canvas)
{
	...
	// Draw Icon
	Canvas.SetPos(TempX, TempY);
	if( Class<MutSR_SRVeterancyTypes>(KFPRI.ClientVeteranSkill)!=None )
		Class<MutSR_SRVeterancyTypes>(KFPRI.ClientVeteranSkill).Static.PreDrawPerk(Canvas,KFPRI.ClientVeteranSkillLevel,M,SM);
	...
	if( CurrentVeterancy!=KFPRI.ClientVeteranSkill || CurrentVeterancyLevel!=LevelIndex )
	{
		...
		lb_PerkEffects.SetContent(Class<MutSR_SRVeterancyTypes>(KFPRI.ClientVeteranSkill).Static.GetVetInfoText(LevelIndex,1));
	}
}
function bool ShowPerkMenu(GUIComponent Sender)
{
	if ( PlayerOwner() != none)
		PlayerOwner().ClientOpenMenu(string(Class'MutSR_SRProfilePage'), false);
	return true;
}

defaultproperties
{
	Begin Object Class=MutSR_SRLobbyFooter Name=BuyFooter
	...
	Begin Object Class=MutSR_SRLobbyChat Name=ChatBox
